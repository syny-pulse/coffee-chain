...
@section('content')
    ...
    <div class="content-section fade-in">
        ...
        <form action="{{ route('processor.order.farmer_order.update', $order->order_id) }}" method="POST" class="form-container">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="farmer_company_id">Farmer</label>
                <input type="text" class="form-control" value="{{ $order->farmer->company_name ?? 'Unknown Farmer' }}" readonly style="background: rgba(111, 78, 55, 0.1);">
                <input type="hidden" name="farmer_company_id" value="{{ $order->farmer_company_id }}">
            </div>

            <div class="form-group">
                <label for="coffee_variety">Coffee Variety</label>
                <input type="text" class="form-control" value="{{ ucfirst($order->coffee_variety) }}" readonly style="background: rgba(111, 78, 55, 0.1);">
                <input type="hidden" name="coffee_variety" value="{{ $order->coffee_variety }}">
            </div>

            <div class="form-group">
                <label for="processing_method">Processing Method</label>
                <input type="text" class="form-control" value="{{ ucfirst($order->processing_method) }}" readonly style="background: rgba(111, 78, 55, 0.1);">
                <input type="hidden" name="processing_method" value="{{ $order->processing_method }}">
            </div>

            <div class="form-group">
                <label for="grade">Grade</label>
                <input type="text" class="form-control" value="{{ ucfirst(str_replace('_', ' ', $order->grade)) }}" readonly style="background: rgba(111, 78, 55, 0.1);">
            ...
            <div class="form-group">
                ...
                <input type="number" name="quantity_kg" id="quantity_kg" class="form-control" required 
                       min="0.01" step="0.01" value="{{ $order->quantity_kg }}"
            ...
            <div class="form-group">
                ...
                <input type="number" name="unit_price" id="unit_price" class="form-control" required 
                       min="0.01" step="0.01" value="{{ $order->unit_price }}"
            ...
            <div class="form-group">
                ...
                <input type="number" name="total_amount" id="total_amount" class="form-control" readonly 
                       step="0.01" value="{{ $order->total_amount }}"
            ...
            <div class="form-group">
                ...
                <input type="date" name="expected_delivery_date" id="expected_delivery_date" 
                       class="form-control" required value="{{ $order->expected_delivery_date ? $order->expected_delivery_date->format('Y-m-d') : '' }}">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" name="actual_delivery_date" id="actual_delivery_date" 
                       class="form-control" value="{{ $order->actual_delivery_date ? $order->actual_delivery_date->format('Y-m-d') : '' }}">
