                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                    <h4 class="mb-sm-0">Customers Balance Report</h4>
...
<div class="table-responsive">
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <td class="text-center" style="background-color: #f2a4a4; color:black"><strong>Due Amount  </strong>
            </td>
      ...
      @php
        ...
        {{-- <td class="text-center"> {{  $item[->product_name] }} </td> --}}
        <td class="text-center">{{ \Str::limit($item->invoice->invoiceDetails->pluck('product.name')->join(', '), 30) }}</td>  <!-- Access product names -->
        {{-- <td class="text-center">{{ $item->invoice->invoiceDetails->pluck('product.name')->join(', ') }}</td>  <!-- Access product names --> --}}
        {{-- {{ \Str::limit($item->invoice->invoiceDetails->pluck('product.name')->join(', '), 20) }} --}}


        {{-- <td class="text-center"> {{  $item->selling_qty }} </td> --}}
        <td class="text-center">{{ $item->invoice->invoiceDetails->pluck('selling_qty')->join(', ') }}</td>  <!-- Access quantities -->

        {{-- <td class="text-center"> {{  'KSh ' . number_format($item->unit_price) }} </td> --}}
        <td class="text-center">{{ $item->invoice->invoiceDetails->pluck('unit_price')->map(fn($price) => 'KSh ' . number_format($price, 0))->join(', ') }}</td> <!-- Access unit prices -->

        {{-- <td class="text-center"> {{  'KSh ' . number_format($item->discount_amount) }} </td> --}}
        <td class="text-center">{{ 'KSh ' . number_format($item->discount_amount)}}</td> <!-- Access discounts -->

        <td class="text-center"> {{  'KSh ' . number_format($item->total_amount) }} </td>
        <td class="text-center"> {{  'KSh ' . number_format($item->paid_amount) }} </td>
        <td class="text-center" style="background-color: #f2a4a4; color:black"> {{ 'KSh ' . number_format($item->due_amount) }} </td>
        ...
        @endforeach
            ...
            <tr>
                ...
                <td class="no-line text-center">
                    ...
                    <strong><span class="badge bg-success" style="font-size: 14px;">Total Outstanding Balances</span></strong>
                </td>
